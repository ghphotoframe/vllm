...
class BailingMTPVariant:
    error_name: str
    attention_cls: type[nn.Module]
    mlp_cls: type[nn.Module]
    expert_mapping_fn: Callable[[nn.Module], list[tuple[str, str, int, str]]]
    mask_zero_position: bool = False
    normalize_attention_dense: bool = False
...
class BailingMultiTokenPredictorLayer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.self_attn = variant.attention_cls(
            config,
            quant_config=vllm_config.quant_config,
            layer_id=layer_id,
        ...
        self._mask_zero_position = variant.mask_zero_position
    ...
    def forward(
        ...
        if self._mask_zero_position:
            inputs_embeds = torch.where(positions.unsqueeze(-1) == 0, 0,
                                        inputs_embeds)
...
class BailingMultiTokenPredictor(nn.Module):
    ...
    def compute_logits(
        ...
        return self.logits_processor(head,
                                     mtp_layer.shared_head(hidden_states))


class BailingMTPModelBase(nn.Module, SupportsPP):
    ...
    def get_expert_mapping(self) -> list[tuple[str, str, int, str]]:
        return self._variant.expert_mapping_fn(self)
    ...
    def load_weights(self, weights: Iterable[tuple[str,
        ...
        def get_spec_layer_idx(name: str) -> int | None:
            if not name.startswith("model.layers."):
                return None
            try:
                layer_idx = int(name.split("model.layers.", 1)[1].split(".",
                                                                        1)[0])
        ...
        def normalize_name(name: str) -> str:
            if self._variant.normalize_attention_dense:
                name = name.replace(".attention.dense", ".self_attn.o_proj")
            name = name.replace(".attention.", ".self_attn.")
            return name.replace("mlp.gate.e_score_correction_bias",
            ...
            if "mlp.experts" in name:
                for mapping in expert_params_mapping:
                    ...
                    if weight_name not in name:
                        continue
                    mapped_name = name.replace(weight_name, param_name)
                    if load_param(mapped_name, loaded_weight,
                                  (expert_id, shard_id)):
                        loaded = True
                        break
                if loaded:
        ...
        for layer_idx in range(
            ...
            if layer_idx not in loaded_mtp_layers:
                raise ValueError(
                    f"{self._variant.error_name} MTP speculative decoding "
                    f"layer {layer_idx} weights are missing from checkpoint. "
